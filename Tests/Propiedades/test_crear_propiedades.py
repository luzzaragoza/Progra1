...
import builtins
import os
from Propiedades import crear_propiedad
...
def test_crear_propiedad_basico(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(crear_propiedad, "cargar_propiedades", lambda: {})
    monkeypatch.setattr(crear_propiedad, "guardar_propiedades", lambda props: props)
    ...
    monkeypatch.setattr(crear_propiedad, "tipo_propiedad", lambda op: "Departamento")

    # 5️⃣ Ejecutamos la función
    resultado = crear_propiedad.crear_propiedad(103)
